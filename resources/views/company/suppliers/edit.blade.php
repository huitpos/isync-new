        ...
        <div class="card-body py-4">
            <form class="mt-3" action="{{ route('company.suppliers.update', ['companySlug' => $company->slug, 'supplier' => $supplier->id]) }}" method="POST" novalidate enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input value="{{ old('tin') ?? $supplier->tin }}" name="tin" type="text" class="form-control @error('tin') is-invalid @enderror" placeholder="TIN" required/>
                ...
                <div class="mb-4">
                    ...
                    <select id="supplier_term_id" name="supplier_term_id" class="form-control @error('supplier_term_id') is-invalid @enderror" required>
                        @foreach($company->supplierTerms as $supplierTerm)
                            <option value="{{ $supplierTerm->id }}" {{ old('supplier_term_id') == $supplierTerm->id || $supplier->supplier_term_id == $supplierTerm->id ? 'selected' : '' }}>{{ $supplierTerm->name }}</option>
                        @endforeach
                    </select>
