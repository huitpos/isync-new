<x-default-layout>
    ...
    <div class="card">
        <div class="card-body py-4">
            <form class="mt-3" action="{{ route('company.payment-types.update', ['companySlug' => $company->slug, 'payment_type' => $paymentType->id]) }}" method="POST" novalidate enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select id="status" name="status" class="form-control @error('status') is-invalid @enderror" required>
                        <option value="active" {{ old('status') == 'active' || $paymentType->status == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status') == 'inactive' || $paymentType->status == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input value="{{ old('name') ?? $paymentType->name }}" name="name" type="text" class="form-control @error('name') is-invalid @enderror" placeholder="Name" required/>
                ...
                <div class="mb-4">
                    ...
                    <input value="{{ old('description') ?? $paymentType->description }}" name="description" type="text" class="form-control @error('description') is-invalid @enderror" placeholder="Description"/>
                ...
                <div class="mb-4 mt-5">
                    <input {{ $paymentType->is_ar ? 'checked' : '' }} value="1" name="is_ar" class="form-check-input" type="checkbox" id="is_ar">
                    ...
                    <div class="repeater">
                        ...
                        <div class="form-group">
                            <div data-repeater-list="payment_type_fields">
                                ...
                                @else
                                    @php
                                        $fromOld = !empty(old('payment_type_fields'));
                                        $fields = old('payment_type_fields') ?? [];


                                        if (!$fromOld) {
                                            $fields = $paymentType->fields->toArray();
                                        }
                                                        ...
                                                        <div data-repeater-list="options" class="mb-5">
                                                            ...
                                                            @else
                                                                @foreach ($field['options'] as $option)
                                                                    <div data-repeater-item>
                                                                        <label class="form-label">Option:</label>
                                                                        <input value="{{ $option['option'] ?? $option }}" name="option" class="form-control mb-2 mb-md-0 @error('payment_type_fields.' . $key . '.options') is-invalid @enderror" placeholder="Option"/>
