<x-default-layout>
    ...
    @section('breadcrumbs')
        {{ Breadcrumbs::render('branch.productDisposals.index', $company, $branch) }}
    @endsection
        ...
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                ...
                <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                    <a href="{{ route('branch.product-disposals.create', ['companySlug' => $company->slug, 'branchSlug' => $branch->slug]) }}" class="btn btn-primary">
                        {!! getIcon('plus', 'fs-2', '', 'i') !!}
                        Add Product Disposal
                    </a>
                </div>
            </div>
        </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            var table = $('#clusters-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{!! route('branch.product-disposals.index', ['companySlug' => $company->slug, 'branchSlug' => $branch->slug]) !!}',
                    data: function (d) {
                        d.status = $('#status').val();
                    }
                },
