...
class SubCategorySalesReportExport implements FromCollection, WithHeadings, WithMapping, WithCustomStartCell, WithEvents, ShouldAutoSize, WithColumnFormatting
{
    protected $branchId;
    protected $startDate;
    protected $endDate;
    ...
    {
        // Get subcategory sales data using raw SQL matching the transactional_db structure
        ...
        // Calculate totals
        $this->totalRegularSales = $subcategoryData->sum('regular_sales');
        $this->totalDiscountSales = $subcategoryData->sum('discount_sales');
        
        // Add percentage to each subcategory
        $subcategoryData->transform(function($item) {
        ...
        $subcategoryData->push((object)[
            'subcategory' => 'TOTAL',
            'quantity_sold' => $subcategoryData->sum('quantity_sold'),
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $event->sheet->setCellValue('A4', 'Sub Category Sales Report');
