...
class HourlyTransactionReportExport implements FromCollection, WithEvents, WithCustomStartCell, ShouldAutoSize
{
    protected $branchId;
    protected $startDate;
    protected $endDate;
    ...
    public function collection()
    {
        ...
        foreach ($transactionsData as $transaction) {
            $this->transactionsByHour[$transaction->transaction_date][intval($transaction->transaction_hour)] = $transaction->transaction_count;
        }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $event->sheet->setCellValue('A4', 'Hourly Transaction Report');
                ...
                foreach ($this->days as $day) {
                    ...
                    // Fill in transaction count data for each hour
                    for ($hour = 0; $hour < 24; $hour++) {
                        $colLetter = $this->getColumnLetter($hour + 2); // +2 because A and B are used for Date and Days
                        $transactionCount = $this->transactionsByHour[$day['date']][$hour] ?? 0;
                        $sheet->setCellValue($colLetter . $rowIndex, $transactionCount > 0 ? $transactionCount : '');
                    }
